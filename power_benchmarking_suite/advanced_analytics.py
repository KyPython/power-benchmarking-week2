...
class AdvancedAnalytics:
    ...
    def compare_profiles(self, profile1: Dict, profile2: Dict) -> Dict:
        ...
        comparison = {
            "power_difference_mw": profile2.get("mean_power_mw", 0)
            - profile1.get("mean_power_mw", 0),
            "power_difference_percent": (
                (profile2.get("mean_power_mw", 0) - profile1.get("mean_power_mw", 0))
                / profile1.get("mean_power_mw", 1)
            )
            * 100,
            "efficiency_improvement": (
                profile1.get("mean_power_mw", 0) / profile2.get("mean_power_mw", 1)
                if profile2.get("mean_power_mw", 0) > 0
                else 0
            ),
            "recommendation": None,
        }
        ...
        if comparison["power_difference_percent"] < -10:
            comparison["recommendation"] = (
                f"Profile 2 is {abs(comparison['power_difference_percent']):.1f}% more efficient. Consider adopting Profile 2's configuration."
            )
        elif comparison["power_difference_percent"] > 10:
            comparison["recommendation"] = (
                f"Profile 1 is {comparison['power_difference_percent']:.1f}% more efficient. Consider keeping Profile 1's configuration."
            )
        else:
            comparison["recommendation"] = (
                "Both profiles have similar power consumption. Choose based on other factors (latency, accuracy, etc.)."
            )
