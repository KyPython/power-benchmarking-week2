...
def generate_synthetic_workload(
    ...
    power_values = np.concatenate([
        np.full(num_drops, low_power),
        np.full(num_active, high_power)
    ])
...
def find_detection_threshold(
    ...
    high_power: float,
    median_power: float,
    threshold_divergence: float = 0.01
    ...
    for _ in range(50):  # Max iterations
        ...
        # Generate workload
        power_values = generate_synthetic_workload(
            low_power, high_power, mid_f, duration=60
        )
...
def main():
    ...
    parser.add_argument(
        ...
        type=float,
        default=1500.0,
        help='Low power during drops (mW, default: 1500)'
    )
    parser.add_argument(
        '--high-power',
        type=float,
    ...
    parser.add_argument(
        ...
        type=float,
        default=2000.0,
        help='Target median power (mW, default: 2000)'
    )
    parser.add_argument(
        '--threshold',
        type=float,
    ...
    for result in threshold_results:
        print(f"{result['divergence_threshold']:>6.1f}%       "
    ...
    print(f"    Mean = ({args.low_power:.1f} × {threshold_f:.3f}) + ({args.high_power:.1f} × {1-threshold_f:.3f})")
