...
"""
    ...
    return subprocess.Popen(
        ["python3", "-c", virus_script], stdout=subprocess.PIPE, stderr=subprocess.PIPE
    )
...
def calculate_attribution_accuracy(baseline: Dict, virus_power: Dict, virus_pids: List[int]):
    ...
    attribution_ratio = (
        estimated_process_power / (estimated_process_power + unattributed)
        if (estimated_process_power + unattributed) > 0
        else 0
    )
...
def main():
    ...
    parser.add_argument(
        "--cores", type=int, default=1, help="Number of CPU cores to stress (default: 1)"
    )
