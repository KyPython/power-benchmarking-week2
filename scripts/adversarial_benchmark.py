...
class AdversarialBenchmark:
    ...
    def _generate_report(self) -> Dict:
        ...
        if not self.signal_times:
            return {
                'status': 'no_signals',
                'message': 'No signals received during benchmark'
            }
    ...
    def print_report(self, report: Dict):
        ...
        for i, sig_info in enumerate(report['all_signals'], 1):
            print(f"   {i}. {sig_info['name']}: {sig_info['elapsed_ms']:.1f} ms ({sig_info['priority']})")
        print()
...
def main():
    parser = argparse.ArgumentParser(
        description='Adversarial benchmark: extreme stress test'
    )
    parser.add_argument(
        '--stress-cores',
        type=int,
        default=4,
        help='Number of P-cores to stress (default: 4)'
    )
    parser.add_argument(
        '--stress-duration',
        type=int,
