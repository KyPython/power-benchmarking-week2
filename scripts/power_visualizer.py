...
def create_power_graph(
    df: pd.DataFrame, output_path: Path, title: str = "Power Consumption Over Time"
):
    """
    ...
    if len(available_columns) > 1:
        ...
        bars = ax_stats.bar(
            labels, means, color=bar_colors, alpha=0.7, edgecolor="black", linewidth=1.5
        )
    ...
    if "total_power_mw" in df.columns and df["total_power_mw"].notna().any():
        ...
        if high_power > low_power:
            ...
            # f = (mean - H) / (L - H)
            drop_fraction = (
                (mean_power - high_power) / (low_power - high_power)
                if (low_power - high_power) != 0
                else 0
            )
        ...
        # Histogram
        ax_dist.hist(
            power_data, bins=30, color="#C73E1D", alpha=0.7, edgecolor="black", linewidth=1
        )
        ax_dist.axvline(
            mean_power, color="red", linestyle="--", linewidth=2, label=f"Mean: {mean_power:.0f} mW"
        )
        ax_dist.axvline(
            median_power,
            color="blue",
            linestyle="--",
            linewidth=2,
            label=f"Median: {median_power:.0f} mW",
        )
        ...
        ax_dist.set_title(
            "Power Distribution + Divergence Dashboard", fontsize=12, fontweight="bold"
        )
...
def print_summary_table(stats: dict):
    ...
    for col, data in stats.items():
        ...
        print(
            f"{name:<20} {data['mean']:>10.2f} {data.get('median', 0):>10.2f} {div_str:<12} {drop_frac_pct:>8.2f}%"
        )
...
def main():
    """Main function."""
    parser = argparse.ArgumentParser(description="Visualize power consumption data from CSV logs")
    parser.add_argument("csv_file", type=str, help="Path to CSV file with power data")
    parser.add_argument(
        "--output", "-o", type=str, help="Output directory for graphs (default: same as CSV)"
    )
    parser.add_argument(
        "--show", action="store_true", help="Display graph in window (in addition to saving)"
    )
