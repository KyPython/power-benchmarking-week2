...
class ThermalThrottleController:
    ...
    def run_thermal_control(self, duration: int = 60, check_interval: float = 2.0) -> Dict:
        """
            ...
            while self.running and (time.time() - start_time) < duration:
                ...
                if burst_fraction > self.target_burst_fraction:
                    ...
                    if not is_regular_pattern:
                        ...
                        print(
                            f"[{time.time() - start_time:.1f}s] Burst: {burst_fraction*100:.1f}% "
                    ...
                    if self.throttled_pids:
                        ...
                        if new_throttle < current_throttle:
                            ...
                            throttle_level = max(
                                new_throttle, current_throttle * 0.95
                            )  # 5% reduction per step
                            ...
                            if throttle_level <= annoyance_threshold:
                                print(
                                    f"     ⚠️  Approaching 'Threshold of Annoyance' ({throttle_level*100:.0f}% CPU)"
                                )
                                ...
                                print(
                                    f"        • Mouse/keyboard activity spike (correlated with throttle)"
                                )
                            ...
                            print(
                                f"     (Gradual throttling: {current_throttle*100:.0f}% → {throttle_level*100:.0f}% "
                    ...
                    if self.target_burst_fraction < 0.12:
                        heartbeat_change = (
                            " (Hot room: Using longer idle periods to prevent thermal death spiral)"
                        )
...
def main():
    ...
    parser.add_argument("app", help='Application name to throttle (e.g., "Python", "Safari")')
    parser.add_argument(
        "--component",
        choices=["ane", "gpu", "cpu"],
        default="ane",
    ...
    parser.add_argument(
        "--duration", type=int, default=60, help="Duration to run (seconds, default: 60)"
    )
    parser.add_argument(
        "--check-interval", type=float, default=2.0, help="Check interval (seconds, default: 2.0)"
    )
    ...
    results = controller.run_thermal_control(
        duration=args.duration, check_interval=args.check_interval
    )
