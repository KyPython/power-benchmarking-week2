...
def get_process_power(
    app_name: str, duration: int = 10, sample_interval: int = 500, use_pid_filter: bool = True
) -> Dict[str, float]:
    """
    ...
    power_values = {"ane": [], "cpu": [], "gpu": [], "total": []}
    ...
    try:
        ...
        for line in stdout.split("\n"):
            ...
            # CPU Power
            cpu_match = re.search(
                r"(?:CPU|Package)\s+Power[:\s]+([\d.]+)\s*mW", line, re.IGNORECASE
            )
            if cpu_match:
            ...
            total_match = re.search(
                r"Total\s+Package\s+Power[:\s]+([\d.]+)\s*mW", line, re.IGNORECASE
            )
            if total_match:
        ...
        for app in apps:
            if app in results:
                ...
                if mean_key in stats:
                    print(
                        f"{app:<20} {stats[mean_key]:>10.2f} {stats.get(min_key, 0):>10.2f} "
...
def main():
    ...
    parser.add_argument(
        "apps", nargs="+", help="Application names to compare (e.g., Safari Chrome)"
    )
    parser.add_argument(
        "--duration",
        "-d",
    ...
    parser.add_argument("--output", "-o", type=str, help="Output CSV file to save results")
    parser.add_argument(
        "--no-pid-filter",
        action="store_true",
