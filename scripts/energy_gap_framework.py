...
def calculate_thermal_throttle_risk(
    ...
    average_power_risk = (
        "LOW"
        if average_power_mw < thermal_threshold_mw * 0.7
        else "MEDIUM" if average_power_mw < thermal_threshold_mw * 0.9 else "HIGH"
    )

    peak_power_risk = (
        "LOW"
        if peak_power_mw < thermal_threshold_mw
        else "MEDIUM" if peak_power_mw < thermal_threshold_mw * 1.2 else "HIGH"
    )
    ...
    if average_power_risk == "HIGH" or (
        peak_power_risk == "HIGH" and throttle_probability == "HIGH"
    ):
        overall_risk = "HIGH"
...
def calculate_work_density(
    instruction_count: int, execution_time: float, total_energy_mj: float
) -> Dict:
    """
    ...
    energy_per_instruction = (
        total_energy_mj / instruction_count if instruction_count > 0 else 0
    )  # mJ per instruction

    # Thermal accumulation factor (energy density × duration)
    thermal_accumulation = (
        energy_density * execution_time
    )  # mJ (total energy, same as total_energy)
...
def calculate_battery_life_advantage(
    ...
    new_battery_life_hours = (
        battery_capacity_mj / new_energy_per_hour_mj if new_energy_per_hour_mj > 0 else float("inf")
    )

    # Battery life extension
    battery_life_extension_hours = new_battery_life_hours - current_battery_life_hours
    battery_life_extension_percent = (
        battery_life_extension_hours / current_battery_life_hours
    ) * 100
    ...
    charging_sessions_saved_per_month = (
        (30 / days_between_charges_current) - (30 / days_between_charges_new)
        if days_between_charges_current > 0
        else 0
    )
    time_saved_from_charging_hours = charging_sessions_saved_per_month * (
    ...
    time_saved_per_day_hours = (
        (battery_life_extension_hours * (charging_time_minutes / 60)) / days_between_charges_new
        if days_between_charges_new > 0
        else 0
    )
    ...
    advantage_over_industry_percent = (
        advantage_over_industry_hours / industry_benchmark_hours
    ) * 100
    ...
    return {
        ...
        "competitive_advantage": {
            ...
            "market_positioning": (
                "SUPERIOR" if advantage_over_industry_hours > 0 else "COMPETITIVE"
            ),
        },
        "energy_metrics": {
...
def calculate_environmental_roi(
    ...
    environmental_roi_kg_per_hour = (
        annual_co2_saved_kg / engineering_hours if engineering_hours > 0 else 0
    )
    ...
    carbon_payback_days = (
        (engineering_carbon_kg / annual_co2_saved_kg) * 365
    ...
    # Years to offset
    years_to_offset = (
        engineering_carbon_kg / annual_co2_saved_kg if annual_co2_saved_kg > 0 else float("inf")
    )

    # Priority score (higher = better)
    priority_score = environmental_roi_kg_per_hour * np.log10(
        tasks_per_day + 1
    )  # Log scale for tasks
...
def prioritize_backlog_by_sustainability(optimization_tasks: List[Dict]) -> Dict:
    """
    ...
    return {
        ...
        "average_environmental_roi": (
            np.mean([t["environmental_roi_kg_per_hour"] for t in prioritized_tasks])
...
def evaluate_sustainability_vs_performance(
    ...
    # Normalize weights
    total_weight = (
        corporate_esg_weight + user_experience_weight + financial_weight + environmental_weight
    )
    if total_weight != 1.0:
...
def calculate_long_term_performance_play(
    performance_task: Dict, sustainability_task: Dict, time_horizon_years: float = 3.0
) -> Dict:
    """
...
def generate_battery_life_whitepaper_proof(
    ...
    ghost_energy_percent = (
        ghost_energy_eliminated_mj / fast_clocked_metrics.get("total_energy_mj", 1)
    ) * 100
    ...
    efficiency_improvement_ratio = (
        fast_energy_per_work / stall_free_energy_per_work if stall_free_energy_per_work > 0 else 0
    )
